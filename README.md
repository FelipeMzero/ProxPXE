...
4. Baixa o template oficial Debian 12, configura o bootloader GRUB2 Netboot e inicia todos os serviços.
...
- **Aplicação 100% em Shell Script (Bash):**
  ...
  - Scripts modulares: `pxe-scan.sh`, `pxe-theme.sh`, `pxe-watch.sh` e `pxe-cli.sh`.
- **Tela de Boot Estilo Ventoy:**
  ...
  - Logotipo centralizado, papel de parede moderno, barra de contagem regressiva e atalhos de teclado.
...
- **Modo Seguro ProxyDHCP (Porta 4011):**
  ...
  - Zero risco de conflito de IP na sua rede!
...
| `/data/theme/logo.png` | Sua imagem de Logotipo exibida no topo do menu de boot. | `.png` (260x68 aprox.) |
...
Como o sistema opera em **ProxyDHCP**, você não precisa configurar nada no seu roteador:
...
4. O menu gráfico do Ventoy será carregado com a sua logo e a lista de ISOs para iniciar!

---

## 🛠️ Ferramenta de Linha de Comando (`pxe-cli`)

Dentro do container, você conta com um comando CLI exclusivo para gerenciar o PXE:
...
│   └── pxe-cli.sh           # Interface de linha de comando
├── web/
│   └── index.html           # Painel Web com Simulador Ventoy Full HD
├── configs/
